...
} from "./parials/notification.utils";
...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({
	...
	useEffect(() => {
		try {
			const ev = new CustomEvent("notifications:count", {
	...
	const handleNewNotification = useCallback((newNotif: Notification) => {
		...
		antNotification.open({
			message: (
				<span className="font-semibold text-gray-900">New notification</span>
			),
			description: (
				<div>
					<p
	...
	const handleNotificationClick = async (notif: Notification) => {
		...
		// Navigate to the route based on notification type
						...
						n.notifId === notif.notifId
							? {
									...
									invitation: n.invitation
										? { ...n.invitation, status: newStatus as any }
										: null,
							  }
							: n
					)
				...
				antNotification[action === "accept" ? "success" : "info"]({
					message:
						action === "accept"
							...
							: t("notification.invitationDeclined"),
					description:
						action === "accept"
	...
	const content = (
		<div
			className="w-[360px] flex flex-col bg-white rounded-lg shadow-sm font-sans h-full max-h-[80vh]"
		...
		>
			...
			<div
				className="flex-1 overflow-y-auto max-h-[60vh] custom-scrollbar flex flex-col p-2"
				style={{ overflowX: "hidden", boxSizing: "border-box" }}
			>
				{loading && notifications.length === 0 ? ( // Chỉ hiện loading khi chưa có dữ liệu
					...
					filteredNotifications.map((notif) => (
						<NotificationItem
							...
							onAccept={(n, e) => {
								e.stopPropagation();
								handleInvitationAction(n, "accept");
							}}
							onDecline={(n, e) => {
								e.stopPropagation();
								handleInvitationAction(n, "decline");
							}}
							isResponding={
	...
	return (
		<div className={className} style={style}>
			...
			>
				<div
					className={`relative w-9 h-9 rounded-full flex items-center justify-center cursor-pointer transition-colors duration-200 select-none ${
						open
