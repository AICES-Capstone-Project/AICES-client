			...
			try {
				...
				if (catsResp?.data) {
					if (Array.isArray(catsResp.data)) categoriesData = catsResp.data;
						...
						(catsResp.data as any).categories &&
						Array.isArray((catsResp.data as any).categories)
					)
	...
	useEffect(() => {
		...
		const normalizeLanguageValue = (val: any) => {
			...
			const found = languages.find(
				(l: any) =>
					String((l as any).id) === s ||
					String((l as any).languageId) === s ||
					String((l as any).name).toLowerCase() === s.toLowerCase()
			);
			...
			const base: any = {
				...
				employmentTypeIds: Array.isArray(job.employmentTypes)
					? job.employmentTypes.map((t: any) =>
							typeof t === "object" ? t.employTypeId ?? t.name : t
					  )
					: [],
				skillIds: Array.isArray(job.skills)
					? job.skills.map((s: any) =>
							typeof s === "object" ? s.skillId ?? s.name : s
					  )
					: [],
				languageIds: Array.isArray((job as any).languageIds)
					...
					? (job as any).languages.map((l: any) =>
							normalizeLanguageValue(
								typeof l === "object" ? l.id ?? l.languageId ?? l.name : l
							)
					  )
					: [],
				...
				levelId:
					(job as any).levelId ??
					((job as any).level && (job as any).level.id !== undefined
						? (job as any).level.id
						: undefined) ??
					((job as any).level && (job as any).level.levelId !== undefined
						? (job as any).level.levelId
						: undefined) ??
					(job as any).level ??
					undefined,
			...
			if (job.categoryName && categories.length > 0) {
				...
				if (cat) {
					...
					(async () => {
						setLoadingSpecs(true);
						try {
							const resp = await systemService.getSpecializations(
								cat.categoryId
							);
							setSpecializations(resp?.data || []);
								...
								base.employmentTypeIds.forEach((val: any) => {
									if (
										!employmentTypes.find(
											(e: any) =>
												e.employTypeId === val || String(e.name) === String(val)
										)
									) {
										setEmploymentTypes((prev) => [
											...prev,
											{ name: String(val), employTypeId: val },
										]);
									}
								});
							}
							if (Array.isArray(base.skillIds)) {
								base.skillIds.forEach((val: any) => {
									if (
										!skills.find(
											(s: any) =>
												s.skillId === val || String(s.name) === String(val)
										)
									) {
										setSkills((prev) => [
											...prev,
											{ name: String(val), skillId: val },
										]);
									}
								});
							}
									...
									if (
										!languages.find(
											(l: any) =>
												l.id === val ||
												l.languageId === val ||
												String(l.name) === String(val)
										)
									) {
										setLanguages((prev) => [
											...prev,
											{ name: String(val), id: val },
										]);
									}
								});
							}
								...
								setLevels((prev) => {
									if (
										!prev.find(
											(lv: any) =>
												lv.id === base.levelId ||
												lv.levelId === base.levelId ||
												String(lv.name) === String(base.levelId)
										)
									) {
										return [
											...prev,
											{ name: String(base.levelId), id: base.levelId },
										];
									}
									return prev;
								});
		...
		<Drawer title="Edit Job" width={640} open={open} onClose={onClose}>
			<Form
				...
				onFinish={(values) => {
					...
					// require at least 2 criteria and max 19
					if (crit.length < 2) {
						form.setFields([
							{
								name: ["criteria"],
								errors: ["At least 2 criteria must be provided."],
							},
						]);
						return;
					}
					if (crit.length > 19) {
						form.setFields([
							{
								name: ["criteria"],
								errors: ["Maximum of 19 criteria can be provided."],
							},
						]);
						return;
					}

					// Validate criteria names are unique (case-insensitive)
					const criteriaNames = crit.map((c: any) =>
						String(c?.name || "")
							.trim()
							.toLowerCase()
					);
					const duplicateNames = criteriaNames.filter(
						(name: string, index: number) =>
							name && criteriaNames.indexOf(name) !== index
					);
						...
						form.setFields([
							{
								name: ["criteria"],
								errors: [
									"Criteria cannot have the same name. Please use unique names.",
								],
							},
						]);
						return;
					...
					const totalImportance = crit.reduce(
						(acc: number, c: any) => acc + Number(c?.importance || 0),
						0
					);
					...
					const unrounded = crit.map(
						(c: any) => Number(c.importance || 0) / totalImportance
					);
					...
					const nonNumericEmp =
						Array.isArray(payload.employmentTypeIds) &&
						payload.employmentTypeIds.some(
							(v: any) => v !== null && v !== undefined && isNaN(Number(v))
						);
					const nonNumericSkill =
						Array.isArray(payload.skillIds) &&
						payload.skillIds.some(
							(v: any) => v !== null && v !== undefined && isNaN(Number(v))
						);
						...
						form.setFields([
							{
								...
								errors: nonNumericEmp
									? ["Some employment types could not be resolved to IDs"]
									: [],
							},
							{
								name: ["skillIds"],
								errors: nonNumericSkill
									? ["Some skills could not be resolved to IDs"]
									: [],
							},
						]);
						return;
					...
					const totalWeight = processedCriteria.reduce(
			...
			>
						...
						{(fields, { add, remove }) => (
							<>
								{fields.map((field, index) => {
									...
									const totalImportance = Array.isArray(critList)
										? critList.reduce(
												(s: number, it: any) => s + Number(it?.importance || 0),
												0
										  )
										: 0;
									...
									const estimatedPercent =
										totalImportance > 0
											? (currentImportance / totalImportance) * 100
											: 0; // Dynamic placeholder based on index
									const placeholder =
										index === 0
											? "Experience"
											: index === 1
										...
										>
											...
											>
												<Form.Item
													...
													rules={[
														{
															required: true,
															message: "Please input criteria name",
														},
														{
															max: 70,
															message:
																"Criteria name must not exceed 70 characters",
														},
													]}
													style={{ flex: 1, marginBottom: 0 }}
												>
													<Input placeholder={placeholder} maxLength={70} />
												</Form.Item>
												<div
													style={{
														width: 92,
											...
											<Form.Item
												...
												rules={[
													{
														required: true,
														message: "Please input importance",
													},
												]}
								...
								>
									2-19 criteria required. Each criteria name must be unique.
									Importance values will be normalized to weights on save.
								</div>
							</>
						)}
					</Form.List>
				...
				<Form.Item
					...
					rules={[
						{
							required: true,
							message: "At least one Employment Type ID is required",
						},
						{
							type: "array",
							min: 1,
							message: "At least one employment type ID must be provided.",
						},
					]}
				>
					<Select
				...
				<Form.Item name="skillIds" label="Skills (Optional)">
					<Select
						className="company-select"
						mode="multiple"
				...
				<div style={{ display: "flex", gap: "16px" }}>
					<Form.Item
						...
						label="Languages"
						style={{ width: "50%" }}
						rules={[{ required: false, message: "Please select languages" }]}
					>
						<Select
							className="company-select"
							mode="multiple"
							...
							maxTagTextLength={12}
							placeholder={
								loadingLangs ? "Loading languages..." : "Select languages"
							}
							loading={loadingLangs}
							options={languages.map((l: any) => ({
								label: l.name,
								value: l.id ?? l.languageId,
							}))}
							allowClear
							style={{ minHeight: 40 }}
						/>
					...
					<Form.Item
						...
						label="Level"
						style={{ width: "50%" }}
						rules={[{ required: false, message: "Please select a level" }]}
					>
						<Select
							className="company-select"
							size="middle"
