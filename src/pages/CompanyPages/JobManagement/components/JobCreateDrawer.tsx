...
const JobCreateDrawer = ({ open, onClose, onSubmit, saving }: Props) => {
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				const catsResp = await categoryService.getAll({
					page: 1,
					pageSize: 100,
				});
				...
				if (catsResp?.data) {
					if (Array.isArray(catsResp.data)) {
						...
						catsResp.data.categories &&
						Array.isArray(catsResp.data.categories)
					) {
	...
	const calculateTotalScore = () => {
		return (criteriaValues || []).reduce(
			(sum: number, item: any) => sum + (item?.importance || 0),
			0
		);
	};
			...
			<Form
				...
				onFinish={async (values) => {
					...
					const totalImportance = crit.reduce(
						(acc: number, c: any) => acc + (c?.importance || 0),
						0
					);
			...
			>
				...
				>
					<Input.TextArea
						...
						maxLength={5000}
						showCount
						placeholder="3+ years experience with C#, .NET, and SQL."
					/>
				</Form.Item>
							...
							<>
								{fields.map((field, index) => {
									...
									const estimatedPercent =
										totalScore > 0
											? Math.round((currentImportance / totalScore) * 100)
											: 0;
				...
				>
					<Select
						...
						options={employmentTypes.map((e: any) => ({
							...
							value:
								e?.employTypeId ?? e?.id ?? e?.value ?? e?.name ?? String(e),
						}))}
					/>
				</Form.Item>
