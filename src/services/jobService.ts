...
export const jobService = {
  ...
  createJob: async (jobData: Partial<CompanyJob>): Promise<ApiResponse<CompanyJob>> => {
    return await post<CompanyJob, Partial<CompanyJob>>(API_ENDPOINTS.COMPANY.GET_JOBS_PUBLIC, jobData);
  },
  ...
  updateJob: async (jobId: number, jobData: Partial<CompanyJob>): Promise<ApiResponse<CompanyJob>> => {
    ...
    return await patch<CompanyJob>(`${API_ENDPOINTS.COMPANY.GET_JOB}/${jobId}`, jobData as Record<string, unknown>);
  },
  ...
  updateJobStatus: async (jobId: number, status: string): Promise<ApiResponse<CompanyJob>> => {
    ...
    return await put<CompanyJob, { status: string }>(`${API_ENDPOINTS.COMPANY.GET_JOB}/${jobId}/status`, { status });
  },
  
  // Delete job by ID
  deleteJob: async (jobId: number): Promise<ApiResponse<null>> => {
    return await remove<null>(`${API_ENDPOINTS.COMPANY.GET_JOB}/${jobId}`);
  },
};
