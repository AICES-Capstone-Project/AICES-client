...
const PricingPage: React.FC = () => {
  ...
  const filteredPlans: PlanType[] = mappedPlans.filter((plan) => {
    ...
    if (currentSubscriptionName && currentSubscriptionName.toLowerCase() !== "free") {
      return plan.title !== currentSubscriptionName && plan.title.toLowerCase() !== "free";
    }
  ...
  return (
    <div className="bg-white text-slate-800 min-h-screen pt-20 pb-24 px-6 md:px-24">
      ...
      {loading ? (
        ...
        <PlansGrid plans={filteredPlans} currentSubscriptionName={currentSubscriptionName} />
      )}
