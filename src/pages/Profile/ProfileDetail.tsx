...
const ProfileDetail: React.FC = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      await profileService.updateMultipart(form);
      message.success("Profile updated successfully");
      dispatch(fetchUser()); // refresh lại thông tin
      // tùy chọn: clear avatar đã chọn
      // setAvatarFile(null);
    } catch (err: any) {
      message.error(err?.response?.data?.message || "Update profile failed");
    } finally {
      setSaving(false);
    }
  };
